...
$errmsg="";
$flag=0;
if((strlen($start)>0 && strlen($end)>0) && strlen($prodcname)>0 && strlen($desc)==0)
{ $sql="SELECT * from product as p,productcategory as pc WHERE p.productcategoryid=pc.productcategoryid AND productprice BETWEEN $start AND $end AND productcategoryname='$prodcname'"; $flag=1;}
if((strlen($start)==0 && strlen($end)==0)  && strlen($prodcname)>0 && strlen($desc)==0)
...
if((strlen($start)>0 && strlen($end)>0) && strlen($prodcname)==0 && strlen($desc)>0)
{ $sql="SELECT * from product as p,productcategory as pc WHERE p.productcategoryid=pc.productcategoryid   AND productname LIKE '%$desc%' AND productprice BETWEEN $start AND $end"; $flag=1;}
if((strlen($start)>0 && strlen($end)>0) && strlen($prodcname)==0 && strlen($desc)==0)
{ $sql="SELECT * from product as p,productcategory as pc WHERE p.productcategoryid=pc.productcategoryid AND productprice BETWEEN $start AND $end "; $flag=1;}
if((strlen($start)==0 && strlen($end)==0) && strlen($prodcname)==0 && strlen($desc)>0)
...
<table width='600' border='5' cellspacing='1' cellpadding='5'>
<tr>
